...
router.get('/', (req, res) => {
    ...
    page.total = getTotal(page.cart);

    Menu.entries().then(menu => {
        page.menu = menu;
    })
    .then(() => Menu.listAll(0)
        .then(menu => {
...
router.get('/jquery/add', (req, res) => {
    ...
    for (let entry in req.session.cart) {
        if (entry === entryId) {
            count = Number(req.session.cart[entry].count) + Number(count);
        }
    }

    Menu.findOne(entryId).then(entry => {

        let newEntry = Object.assign(entry.dataValues, { count });
...
router.post('/pay', (req, res) => {
    console.log(getTotal(req.session.cart));
        ...
        'transactions': [{
            'item_list': {
                'items': [{
                    'name': 'Vincent\'s Bar',
                    'sku': '1610',
                    'price': `${getTotal(req.session.cart)}`,
                    'currency': 'EUR',
                    'quantity': 1
                }]
            },
